                ...
                {skillsFE.slice(0,secondColumnStart).map((item,index)=>{
                    return(        
                     <div className="skills__data" key={index}>
                        {item.name==="React Native"?<Icon icon="tabler:brand-react-native" />:item.name==="Javascript"? <Icon icon="simple-icons:javascript" />:item.name==="HTML"? <Icon icon="akar-icons:html-fill" />:item.name==="HTML"?<Icon icon="teenyicons:git-outline" />:<Icon icon="el:css" />}
                        <div key={item.id}>
